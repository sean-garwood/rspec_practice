...
describe Cat do
  ...
  subject(:calvin) { described_class.new('Calvin', 'Half-Siamese', 'Black') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'When Cat and dog share a method name' do
    include_examples 'shared method name'
  end

  it 'is not hungry' do
    expect(calvin).not_to be_hungry
  end

  it 'is hiding' do
    expect(calvin).to be_hiding
  end
end
